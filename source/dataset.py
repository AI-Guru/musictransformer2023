...
class Dataset:
    ...
    def iterate(self, split:str, shuffle:bool, batch_size:int):
        ...
        for batch in dataset:
            ...
            # Do the token dropout.
            if do_token_dropout_encoder:
                token_mask = torch.rand(encoder_ids.shape) < token_dropout_probability
                print(token_mask)
                encoder_ids = torch.where(torch.rand(encoder_ids.shape) < token_dropout_probability, torch.tensor(mask_tokens).to(encoder_ids.device), encoder_ids)
