...
# TODO exec(open('configurator.py').read()) # overrides from command line or config file
...
def get_batch(split):
    ...
    # Get random indices.
    indices = np.random.randint(0, len(dataset), batch_size).tolist()
    assert len(indices) == batch_size, f"Error: len(indices) != batch_size ({len(indices)} != {batch_size})."
    ...
    if device_type == "cuda":
        encoder_ids = encoder_ids.pin_memory().to(device, non_blocking=True)
        decoder_ids = decoder_ids.pin_memory().to(device, non_blocking=True)
        target_ids = target_ids.pin_memory().to(device, non_blocking=True)
    else:
