...
from nanoGPT.model import LayerNorm, MLP
...
class CausalSelfAttentionNoMaskAndCrossAttention(nn.Module):
    ...
    def forward(self, x, x_cross):
        ...
        # calculate query from x
        q = self.c_attn(x)
        q = q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)

        # calculate keys and values from x_cross
...
class Transformer(nn.Module):
    ...
    def forward(self, encoder_ids, decoder_ids, target_ids=None):
